...
export default function Charts({ processos }: { processos: Processo[] }) {
    ...
    const chartData = useMemo(() => {
        ...
        const sortedMonthLabels = Object.keys(monthlyData).sort((a, b) => { const [m1, y1] = a.split('/'); const [m2, y2] = b.split('/'); const monthsOrder = ['jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez']; const dateA = new Date(Number(`20${y1}`), monthsOrder.indexOf(m1.replace('.',''))); const dateB = new Date(Number(`20${y2}`), monthsOrder.indexOf(m2.replace('.',''))); return dateA.getTime() - dateB.getTime(); });
